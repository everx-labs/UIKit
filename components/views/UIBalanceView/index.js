...
const UIBalanceSymbol = React.memo(
    ({
        animation,
        textStyle,
        fractionalTextStyle,
        ...
        <Text
            style={[
                ...
                textStyle,
                children.kind === 'int' ? null : fractionalTextStyle,
            ]}
        >
            <Animated.Text
                style={[
            ...
            <Text style={{ color: 'transparent' }}>{children.value}</Text>
        </Text>
    ),
    (prev, next) => prev.children === next.children,
);

export default class UIBalanceView extends React.Component<Props, NewState> {
    static defaultProps = {
        separator: '.',
    };

    static processBalanceString(props: Props): BalanceSymbol[] {
        let isSeparatorPassed = false;

        // eslint-disable-next-line arrow-parens
        return props.balance.split('').map(symbol => {
            if (symbol === props.separator) {
                isSeparatorPassed = true;
            }

            return {
                kind: isSeparatorPassed ? 'fraction' : 'int',
                value: symbol,
            };
        });
    }

    static getDerivedStateFromProps(props: Props, state: NewState) {
        return {
            prevBalance: (state || {}).nextBalance,
            nextBalance: UIBalanceView.processBalanceString(props),
        };
    }
    ...
    componentDidUpdate() {
        if (this.props.animated) {
            Animated.parallel(
                // eslint-disable-next-line arrow-parens
                Object.keys(this.animations).map(indexKey => {
    ...
    animations: { [number]: Animated.Value } = {};

    getAnimation(index: number) {
        if (this.animations[index] == null) {
            this.animations[index] = new Animated.Value(0);
        }
    ...
    render() {
        return (
            <View style={[{ overflow: 'hidden' }]}>
                ...
                >
                    {this.state.nextBalance.map((i, index) => (
                        <UIBalanceSymbol
                ...
                {this.state.prevBalance && (
                    <Text style={UIStyle.common.positionAbsolute()}>
                        {this.state.prevBalance.map((i, index) => (
                            <UIBalanceSymbol
                                animation={this.getAnimation(index).interpolate(
                                    {
                                        inputRange: [0, 1],
...
export class UIBalanceView2 extends UIComponent<Props, State> {
    static defaultProps = {
        balance: '',
        description: '',
