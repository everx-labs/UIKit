...
const countries = require('../UIAssets/countries/countries.json');
...
export default class UIFunction {
    ...
    static numberFromMoneyString(string, currency = { code: 'USD', symbol: '$', precision: 0 }) {
        const options = {
            code: currency.code,
    ...
    static moneyStringFromNumber(number, currency = { code: 'USD', symbol: '$', precision: 0 }) {
        const options = {
            code: currency.code,
            symbol: currency.symbol,
            precision: currency.precision || 0,
    ...
    static calculateNewCaretPosition(
        ...
        const textBeforeCurrentCaret
            = this.numericText(
                ...
                { ...currency, precision: precisionBefore },
            );
        let newCaretPos = 0;
    ...
    static numericText(text, currency = { code: 'USD', symbol: '$', precision: 0 }) {
    ...
    static alertObject(obj, level = 0) {
        ...
        if (level === 0) alert(str);
        else return str;
    }

    static repeat(str, number) {
        let res = '';
        for (let i = 0; i < number; i += 1) { res += str; }
        return res;
    }
